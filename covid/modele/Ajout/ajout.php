		...
		if($code==2){
			...
			if($code==4){
				if (isset($_POST['option'])){
					...
					{
						$dates=date('Y/m/d');
						$covid=$_POST['option'][$i];
						$req = $conn->prepare('INSERT INTO covid (date, Id_pa ,id_user_labo, id_user_rec, Description, resultat) VALUES(?, ?, ?, ?, ?, ?)');
						$req->execute(array($dates, $_POST['id_p'], "", $_POST['user'], $covid,""));
					}
       			 }
			}
