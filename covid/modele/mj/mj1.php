...
require ("../Connexion.php");
	...
	{		
		...
		if($code=1)
		{
			...
			$Pr_P=$_POST['PR_P'];
			$Em_P=$_POST['E_P'];
			$rs=$conn->query('SELECT * FROM patient WHERE Nom =\''.$Nom_P.'\'' );
			
			$req = $conn->prepare('UPDATE patient SET Nom = :Nom_P, Post_nom = :Pnom_P, Prenom = :Prnom_P, Sexe = :S_P, DN = :Dn_P, EC = :Ec_P, Adresse = :Ad_P, Phone = :Ph_P, Profession = :Pr_P, Employeur = :Em_P WHERE id_pa = :id_P');
			$req->execute(array(
				'id_P'    =>$id_P,
				'Nom_P'   => $Nom_P,
				'Pnom_P'  => $Pnom_P,
