...
if($_POST)
	{
		...
		if($code==2){
			...
			if($code==4){
					$id_p=$_POST['id_p'];
					$id_user=$_POST['user'];
					$descrip=$_POST['covidDesc'];
					$resul="";
					$req = $conn->prepare('INSERT INTO covid (Id_pa, id_user_labo ,id_user_rec, Description, resultat) VALUES(?, ?, ?, ?, ?, ?)');
					$req->execute(array($id_p,"",$id_user,$descrip,""));
			}
